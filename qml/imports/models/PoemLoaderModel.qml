...
AsemanObject {
    ...
    function getText(cleanText, index) {
        var text = "";
        for (var i=index; i<versesModel.count; i++) {
            var e = versesModel.get(i);
            text += e.text + "\n";
        ...
        if (!cleanText) {
            for (var j=1; j<categoriesModel.count; j++) {
                text += categoriesModel.get(j).title
                if (j < categoriesModel.count-1)
                    text += ", ";
                else
                    text += " - ";
            }
